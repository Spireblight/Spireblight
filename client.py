...
async def main():
    ...
    if not global_config.website_url or not global_config.secret:
        print("Config is not complete")
        time.sleep(3)
        return
    async with ClientSession(global_config.website_url) as session:
        while True:
            time.sleep(timeout)
            start = time.time()
            timeout = 1
            if possible is None:
                for file in os.listdir(os.path.join(global_config.STS_path, "saves")):
            ...
            if possible is not None:
                try:
                    cur = os.path.getmtime(os.path.join(global_config.STS_path, "saves", possible))
                except OSError:
                    possible = None
            ...
            if possible is None: # don't check run files during a run
                for path, folders, _f in os.walk(os.path.join(global_config.STS_path, "runs")):
            ...
            try:
                ...
                if to_send: # send runs first so savefile can seamlessly transfer its cache
                    for path, file, profile in to_send:
                        ...
                        async with session.post("/sync/run", data={"run": content, "name": file, "profile": profile}, params={"key": global_config.secret, "start": start}) as resp:
                            if not resp.ok:
                                all_sent = False
                    if all_sent:
                        last_run = max(files)
                ...
                if possible is None and has_save: # server has a save, but we don't (anymore)
                    async with session.post("/sync/save", data={"savefile": b"", "character": b""}, params={"key": global_config.secret, "has_run": str(all_sent).lower(), "start": start}) as resp:
                        if resp.ok:
                            has_save = False
                    # update all profiles
                    data = {
                    ...
                    with open(os.path.join(global_config.STS_path, "preferences", "STSSaveSlots")) as f:
                    ...
                    for i in range(3):
                        ...
                        if i:
                            name = f"{i}_{name}"
                        try:
                            fname = os.path.join(global_config.STS_path, "preferences", name)
                    ...
                    async with session.post("/sync/profile", data=data, params={"key": global_config.secret, "start": start}) as resp:
                        if resp.ok:
                            lasp = tobe_lasp
                        else:
                ...
                if possible is not None and cur != last:
                    content = ""
                    with open(os.path.join(global_config.STS_path, "saves", possible)) as f:
                        content = f.read()
                    content = content.encode("utf-8", "xmlcharrefreplace")
                    char = possible[:-9].encode("utf-8", "xmlcharrefreplace")
                    async with session.post("/sync/save", data={"savefile": content, "character": char}, params={"key": global_config.secret, "has_run": "false", "start": start}) as resp:
                        if resp.ok:
                            last = cur
                            has_save = True
