...
_consts = {
    "discord": global_config.discord_invite_link,
...
class TwitchConn(TBot):
    ...
    async def eventsub_setup(self):
        ...
        self.live_channels[global_config.channel] = bool(await self.fetch_streams(user_logins=[global_config.channel]))

        try:
            await self.esclient.subscribe_channel_stream_start(broadcaster=global_config.channel)
            await self.esclient.subscribe_channel_stream_end(broadcaster=global_config.channel)
...
class EventSubBot(TBot):
    async def event_eventsub_notification_stream_start(self, evt: StreamOnlineData):
        ...
        try:
            _timers["global"].start(global_config.global_commands, stop_on_error=False)
            _timers["sponsored"].start(global_config.sponsored_commands, stop_on_error=False)
...
class DiscordConn(DBot):
    async def on_message(self, message: discord.Message):
        if message.author == self.user:
            return
        if message.content.startswith(global_config.prefix) or isinstance(message.channel, discord.DMChannel):
            content = message.content.lstrip(global_config.prefix).split()
            if not content:
                return
...
async def _timer(cmds: list[str]):
    chan = TConn.get_channel(global_config.channel)
    if not chan or not TConn.live_channels[global_config.channel]:
        return
    cmd = None
    i = 0
    while cmd is None:
...
async def command_cmd(ctx: ContextType, action: str, name: str, *args: str):
    ...
    args = list(args)
    msg = " ".join(args)
    name = name.lstrip(global_config.prefix)
...
async def help_cmd(ctx: ContextType, name: str = ""):
    ...
    if not name:
        await ctx.send(f"I am {__botname__} v{__version__}, made by {__author__}. I am running on Python "
                       ...
                       f"my source code is available at {__github__} . The website is {global_config.website_url}")
        return
    ...
    cmd = (tcmd or dcmd)
    if cmd:
        await ctx.send(f"Full information about this command can be viewed at {global_config.website_url}/commands/{cmd.name}")
        return

    await ctx.send(f"Could not find matching command. You may view all existing commands here: {global_config.website_url}/commands")
...
async def stream_title(ctx: ContextType):
    ...
    live: list[Stream] = await TConn.fetch_streams(user_logins=[global_config.channel])
...
async def is_seeded(ctx: ContextType, j: Savefile):
    ...
    if j["seed_set"]:
        await ctx.send(f"This run is seeded! See '{global_config.prefix}seed' for the seed.")
    else:
...
async def event_likelihood(ctx: ContextType, j: Savefile):
    ...
    await ctx.send(
        ...
        f"See {global_config.prefix}eventrng for more information."
    )
...
async def _last_run(ctx: ContextType, character: str | None, arg: bool | None):
    ...
    if arg:
        ...
        value = f"lost {value}"
    await ctx.send(f"The last {value}'s history can be viewed at {global_config.website_url}/runs/{latest.name}")
...
async def wall_card(ctx: ContextType):
    ...
    await ctx.send(f"Current card in the {global_config.prefix}hole in the wall for the ladder savefile: {p.hole_card}")
...
async def commands_page(req: Request):
    d = {"prefix": global_config.prefix, "commands": []}
    cmds = set()
    if TConn is not None:
...
async def individual_cmd(req: Request):
    ...
    d["prefix"] = global_config.prefix

    return d

async def Twitch_startup():
    global TConn
    TConn = TwitchConn(token=global_config.oauth, prefix=global_config.prefix, initial_channels=[global_config.channel], case_insensitive=True)
    for cmd in _to_add_twitch:
        TConn.add_command(cmd)
    load()

    esbot = EventSubBot.from_client_credentials(global_config.client_id, global_config.client_secret, prefix=global_config.prefix)
    TConn.esclient = EventSubClient(esbot, global_config.webhook_secret, f"{global_config.website_url}/callback")
    await TConn.eventsub_setup()

    if global_config.global_interval and global_config.global_commands:
        _timers["global"] = _global_timer = routine(seconds=global_config.global_interval)(_timer)
        ...
        if TConn.live_channels[global_config.channel]:
            _global_timer.start(global_config.global_commands, stop_on_error=False)

    if global_config.sponsored_interval and global_config.sponsored_commands:
        _timers["sponsored"] = _sponsored_timer = routine(seconds=global_config.sponsored_interval)(_timer)
        ...
        if TConn.live_channels[global_config.channel]:
            _sponsored_timer.start(global_config.sponsored_commands, stop_on_error=False)
...
async def Discord_startup():
    global DConn
    DConn = DiscordConn(global_config.prefix, case_insensitive=True, owner_ids=global_config.owners, help_command=None)
    for cmd in _to_add_discord:
        DConn.add_command(cmd)
    await DConn.start(global_config.token)
